...
from common.security import generate_session_key

from .models import MeshConfig, RegisterResponse, HeartbeatResponse, ConnectRequest, RobotRequest
...
def register_robot(robot_id: str, req: RobotRequest):
    ...
    registry[robot_id] = RobotSession(req.robot_port)
...
def connect_user_to_robot(robot_id: str, req: ConnectRequest):
    ...
    player_port = get_free_port()

    # 1. Allocate ports
    config = MeshConfig(
        robot_pub_port=session.robot_port,
        player_pub_port=player_port,
        user_pub_port=req.user_pub_port,
